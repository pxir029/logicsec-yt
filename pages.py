...
.cp-best-btn{display:inline-flex;align-items:center;gap:7px;padding:11px 16px;border-radius:12px;border:none;background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;font-size:12.5px;font-weight:800;cursor:pointer;transition:.18s;box-shadow:0 4px 14px rgba(239,68,68,.35);white-space:nowrap}
.cp-best-btn:hover{transform:translateY(-1px);box-shadow:0 6px 18px rgba(239,68,68,.45);filter:brightness(1.05)}
.cp-best-btn:active{transform:translateY(0)}
.cp-best-btn i{font-size:15px}
    ...
    <div class="cp-body">
      ...
      <div class="cp-footer">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
          <button type="button" class="cp-best-btn" onclick="applyBestSettings()" title="تنظیم خودکار بهترین مقادیر برای پایداری و سرعت"><i class="ti ti-sparkles"></i> بهترین تنظیمات خودکار</button>
...
function applyBestSettings(){
  // بهترین ترکیب پایدار برای Railway / CDN + کلاینت‌های رایج
  // پروتکل: xhttp-packet-up (سازگارتر با CDN و Railway)
  const protoCard = document.querySelector('.proto-card[data-val="xhttp-packet-up"]');
  if(protoCard) selectProto('xhttp-packet-up', protoCard);
  else document.getElementById('nl-proto').value = 'xhttp-packet-up';

  document.getElementById('nl-fp').value = 'chrome';
  document.getElementById('nl-alpn-preset').value = 'h2,http/1.1';
  document.getElementById('nl-alpn').value = 'h2,http/1.1';
  document.getElementById('nl-alpn').style.display = 'none';
  document.getElementById('nl-port').value = '443';

  // محدودیت‌ها: نامحدود برای تست سریع (کاربر می‌تونه بعداً عوض کنه)
  document.getElementById('nl-val').value = '';
  document.getElementById('nl-unit').value = 'GB';
  document.querySelectorAll('#quota-chips .chip').forEach(c=>c.classList.remove('active'));
  const q0 = document.querySelector('#quota-chips .chip');
  if(q0) q0.classList.add('active');

  document.getElementById('nl-exp').value = '';
  document.querySelectorAll('#exp-chips .chip').forEach(c=>c.classList.remove('active'));
  const e0 = document.querySelector('#exp-chips .chip');
  if(e0) e0.classList.add('active');
  ...
  const i0 = document.querySelector('#iplimit-chips .chip');
  if(i0) i0.classList.add('active');
  ...
  const s0 = document.querySelector('#speed-chips .chip');
  if(s0) s0.classList.add('active');

  if(!document.getElementById('nl-label').value.trim()){
    document.getElementById('nl-label').value = 'بهترین کانفیگ XHTTP';
  }
  toast('بهترین تنظیمات اعمال شد ✓  (XHTTP packet-up + chrome + h2)','ok');
}
...
function protoChip(p){{
  if(p==='xhttp-stream-one')return '<span class="proto-chip pc-xhttp"><i class="ti ti-bolt"></i> XHTTP (legacy)</span>';
